...
3. End-to-end tests for signup/verification, transaction currency mismatch,
   ...
   kept being billed). **2026-09-15: bank consent E2E added**
   (`frontend/e2e/bank-linking-and-consent.spec.ts`, 2 tests, 13 total across
   5 specs) — the full choose-institution → consent → link → expand
   transactions → sync → unlink flow driven through the real browser UI, and
   a separate lapsed-consent/re-authorize flow (using banking's own
   dev-only `lapse-consent` endpoint, since no real provider webhook exists
   to trigger a lapse). Reaching an entitled state in a browser needed a new
   dev-only `POST /subscriptions/dev-grant-trial` endpoint (is_production-gated,
   hidden from the OpenAPI schema — same pattern as the lapse-consent
   endpoint) since no Stripe test-mode credentials exist for a real checkout
   in E2E either. Still outstanding: signup email-verification itself,
   subscription purchase E2E (still needs real Stripe test-mode credentials,
   none available — the same gap that made the dev-grant endpoint necessary
   for banking's own E2E coverage), and admin console E2E.
