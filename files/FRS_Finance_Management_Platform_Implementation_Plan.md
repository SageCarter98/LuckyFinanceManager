...
|---|---|---|---|---|
| Registration and provisioning | FE-1.1–FE-1.4 | `/signup` | Implemented against real `POST /auth/signup`; no verification/resend flow (no backend support — disclosed, not faked) | API contract, component and E2E tests |
| Authentication | FE-2.1–FE-2.8 | `/login`, session boundary | Implemented against real `POST /auth/login`; no `/verify-email`, `/forgot-password`, `/reset-password`, refresh or rate-limit UI (no backend support — disclosed, not faked) | Enumeration, token, return-route tests |
...
| Profile and preferences | FE-3.1–FE-3.4 | `/settings/profile`, `/settings/notifications` | Read-only real data (`GET /auth/me`, `GET/PATCH /notifications`); no editing (no `PUT` profile endpoint — disclosed) | Preference and re-verification tests |
...
| Transactions | FE-6.1–FE-6.7 | `/transactions` list/entry/import | Implemented against real CRUD + filters + CSV import; no pagination (backend has none) | Balance impact, filters, CSV tests |
...
| Notifications | FE-10.1–FE-10.2 | In-app notification surface | Implemented against real `GET /notifications` + mark-read; no preference storage (no backend field) | Event and preference tests |
...
| Export and deletion | FE-13.1–FE-13.8 | `/settings/data` | Export implemented against real `GET /me/export`; deletion disclosed as unavailable (no backend endpoint) | Async export, typed deletion tests |
