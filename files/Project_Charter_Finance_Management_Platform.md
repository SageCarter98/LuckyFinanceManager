...
| **Benefits ownership and funding** | Operational owner: Freston Kenny Adedeme. Measures: **defined 2026-09-13** — see `Benefits_Register.md` for the full table (user adoption, activation, free-to-paid conversion, retention, reliability), each with a real data source in this codebase. **Target numbers still [SPONSOR TO CONFIRM]** for adoption/activation/conversion/retention — no user base or market data exists to derive them from, and this charter will not invent plausible-sounding figures; a conventional 99% reliability floor is suggested pending override. Default Gate 7 date per PM Framework Class A table: 6 months after Gate 6 (interim review at 3 months), once Gate 6 is reached — not yet applicable. Protected review effort: 2 person-days plus measurement cost, per the same table, now included in the Business Case's assurance-effort line (item #105). |
