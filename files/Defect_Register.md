...
**D4 and D5 both close tracker item #135** ("no open S1 defect") — both
fixed and merged, both verified against `master` directly (D4 via `b3e524c`,
D5 via `3ee56e4`/PR #9), same discipline applied to each: #135 didn't get to
claim either as closed until the fix actually landed, not just existed.
Updated 2026-09-18 during the G4.09 security/privacy review
(`files/Security_Privacy_Review_G4.md`) — the "not yet merged" line above
was stale by four days; PR #9 merged 2026-09-14.
...
| G8 | 11 migrations; schema-rollback and backup/restore mechanisms both now rehearsed (2026-09-13, 2026-09-15) but neither at production scale or under a real incident's time pressure | Low (S3) | Open, disclosed | Acceptable pre-launch; a production-shaped drill belongs in the first post-launch operational review (G5) | `Transition_Support_Rollback_Closure.md` §4 (schema rollback), §4a (backup/restore, added 15 September 2026) |
