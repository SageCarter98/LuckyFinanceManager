...
| **Governance and tolerances** | Class A defaults per PM Framework §2: cost/schedule tolerance ±5%, now operative against the baseline `Business_Case_Finance_Management_Platform.md` §4 established 2026-09-13; zero tolerance for reduced acceptance criteria, unapproved material scope change, unresolved critical defects, unlawful processing, or unaccepted critical risk. **Role separation**: not met on paper (§3.3), but a compensating-assurance arrangement is approved and recorded (`Compensating_Assurance_Role_Separation.md` — Milton, named independent reviewer, scoped and time-limited). **Change authority**: sponsor (Freston Kenny Adedeme), also Executive Authority. **Reporting/escalation**: resolved 2026-09-13 — the Sponsor accepted the ad hoc, event-driven cadence in `Communication_Stakeholder_Plan.md` as the approved adjustment to full Class A cadence, closing the Gate 1 capacity condition. |
